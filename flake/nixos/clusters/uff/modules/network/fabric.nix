...
in {
  ...
  systemd.services.vxlan-setup = {
    ...
    after = ["network-online.target" "br100-fabric-network.service"];
    wants = ["network-online.target"];
    path = [pkgs.iproute2];
    script = ''
      set -euxo pipefail

      # Check if bridge is already configured with our IP
      if ip addr show br100 2>/dev/null | grep -q "192.168.52.1/26"; then
        echo "Bridge br100 already configured with 192.168.52.1/26, skipping setup"
        exit 0
      fi
