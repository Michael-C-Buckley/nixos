...
# to share between NixOS and Darwin
{
  flake.hjemConfig.ssh-pubkeys = {
    users.users = {
      michael.openssh.authorizedKeys.keyFiles = [
      ...
      shawn.openssh.authorizedKeys.keyFiles = [
        "/etc/secrets/shawn/ssh/t14.pub"
        "/etc/secrets/shawn/ssh/gpg1.pub"
        "/etc/secrets/shawn/ssh/gpg2.pub"
      ];
    };
  };
}
